...
const CarBrandList = () => {
  ...
  const handleDeleteConfirm = () => {
    ...
    fetch(api+`carbrand/${selectedCarBrandId}`, requestOptions) 
  ...
  const formattedData = filteredItems.map((carbrand) => ({
    ...
    เครื่องมือ: (
      <div className="flex justify-center items-center space-x-4">
        <button onClick={() => CarBrandView(carbrand.id)} className="text-black hover:text-gray-700">
          <AiOutlineEye className="w-5 h-5"/>
        </button>
        <button onClick={() => CarBrandUpdate(carbrand.id)} className="text-black hover:text-gray-700">
          <AiTwotoneEdit className="w-5 h-5"/>
        </button>
        <button onClick={() => openDeleteModal(carbrand.id)} className="text-black hover:text-gray-700">
        ...
        <div className="px-6 ">
          <div className="flex w-full rounded-md my-4 gap-3">
            ...
            <Popup
              ...
              title={
                <>คุณต้องการ <span style={{ fontWeight: 'bold' }}>ลบข้อมูลยี่ห้อรถ Settings</span> ใช่หรือไม่?</>
              }
